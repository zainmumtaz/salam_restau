...
class HistoryInputView extends StatelessWidget {
  ...
  const HistoryInputView({Key? key, this.animationController, this.animation})
      : super(key: key);
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        if (tx.waveLaunchUrl != null && tx.waveLaunchUrl.toString().isNotEmpty) ...[
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (tm.typeId == 4) ...[
                      ...
                      if (tm.checkoutStatus != null && tm.checkoutStatus.isNotEmpty)
                      _detailRow("Checkout Status", tm.checkoutStatus ?? ""),
                      if (tm.paymentStatus != null && tm.paymentStatus.isNotEmpty)
                      _detailRow("Payment Status", tm.paymentStatus ?? ""),
                      if (tm.waveLaunchUrl != null && tm.waveLaunchUrl.isNotEmpty)
                        _wavedetailRow("Wave Payment URL", tm.waveLaunchUrl!),
                      if (tm.whenCreated != null && tm.whenCreated.isNotEmpty)
                      _detailRow("Created At", tm.whenCreated ?? ""),
                      if (tm.whenExpires != null && tm.whenExpires.isNotEmpty)
                      _detailRow("Expires At", tm.whenExpires ?? ""),
                      if (tm.whenCompleted != null && tm.whenCompleted.isNotEmpty)
                      _detailRow("Completed At", tm.whenCompleted ?? "-"),
                      if (tm.date != null && tm.date.isNotEmpty)
                        _detailRow("Date", tm.date ?? "-"),
                      if (tm.reference != null && tm.reference.isNotEmpty)
  ...
  Widget _wavedetailRow(String title, String value,{String? sign,Color? valueColor}) {
    if(sign==null)
    {
      sign="";
    }
